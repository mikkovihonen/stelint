...
def extract_release_notes(version: str) -> str:
    if not CHANGELOG_PATH.is_file():
        raise SystemExit(f"Error: {CHANGELOG_PATH} not found. Cannot extract release notes for version {version!r}.")
    ...
    if not matches:
        ...
        prefix_pattern = re.compile(rf"^## \[{re.escape(version)}(\.[\d+]*)?\].*?$", re.MULTILINE)
        matches = list(prefix_pattern.finditer(text))

    if not matches:
        raise SystemExit(f"No changelog section found for version {version!r}. Available versions: {', '.join(m.group(1) for m in VERSION_HEADING_RE.finditer(text))}")
